        ...
        this.drow_area.addEventListener("click",()=>{
            ...
            if(Tool.drow == 2){
                if(document.querySelector("#text_s") && document.querySelector("#text_s").value.length > 0){
                    ...
                    Tool.drow_path[View.video_num][Object.keys(Tool.drow_path[View.video_num]).length]={"video":View.video_num,"number":this.id,"id":this.name,"path":{"x":this.tx,"y":this.ty},"width":parseInt((document.querySelector("#text_s").style.width)),"height":this.text_height,"value":document.querySelector("#text_s").value,"text_size":this.text,"type":"text","start_t":0,"keep_t":Track.end_t,"add":[{"num":this.id,"video":View.video_num,"number":Tool.canvas_num[View.video_num],"id":this.name,"path":{"x":this.tx,"y":this.ty},"width":parseInt((document.querySelector("#text_s").style.width)),"height":this.text_height,"value":document.querySelector("#text_s").value,"text_size":this.text,"type":"text","start_t":0,"keep_t":Track.end_t}]}
                    Tool.drow = false;
